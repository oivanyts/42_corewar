...
uint8_t		get_param_type
	(uint8_t opcode, uint8_t tparams, uint8_t param_number)
{
	if (param_number == 1)
	{
		if ((g_op_tab[opcode].targs[param_number - 1] & T_IND)
		&& (((tparams & T_FIRST_PARAM) >> 6) & 0x3) == IND_CODE)
		{
			return (T_IND);
		}
		else if ((g_op_tab[opcode].targs[param_number - 1] & T_REG)
		&& (((tparams & T_FIRST_PARAM) >> 6) & 0x3) == REG_CODE)
		{
			return (T_REG);
		}
		else if ((g_op_tab[opcode].targs[param_number - 1] & T_DIR)
